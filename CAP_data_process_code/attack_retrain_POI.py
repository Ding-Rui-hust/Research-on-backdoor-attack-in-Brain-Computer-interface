...
parser.add_argument('--data', type=str, default='MI_DR', help='name of data, ERN or MI or P300 or MI_DR')
parser.add_argument('--model', type=str, default='DeepCNN', help='name of model, EEGNet or DeepCNN')
parser.add_argument('--a', type=float, default=0.2, help='NPP amplitude')#阶跃变化，过渡区很小
...
# x_validation, y_validation ,  _, _= utils.split_data([x_validation, y_validation], split=0.2, shuffle=True)
...
for i in range(repeat):
    ...
    if model_used == 'EEGNet':
        model = models3.EEGNet(nb_classes=nb_classes, Chans=channels, Samples=samples)
    ...
    #model = load_model(filepath='model_orginal_poison_CNNbeforeDRMI{}_{}_{}.h5'.format(npp_params[0], npp_params[1], npp_params[2]))
...
model.save(filepath='model_orginal_CNNPOI0.2.h5')
#model.save(filepath='model_orginal_poison_CNNbeforeDRMI{}_{}_{}.h5'.format(npp_params[0], npp_params[1], npp_params[2]))
#model.save(filepath='model_orginal_cleanretrain{}_{}_{}.h5'.format(npp_params[0], npp_params[1], npp_params[2]))
